...
export default function TimeInput({dates, onTimeSelected}) {
  ...
  const handleTimeChange = (date: string, type: string, value: string) => {
    ...
    if (numericValue.length <= 4) {
      const newTimes = {
        ...times,
        [date]: {...times[date], [type]: formattedValue},
      };
      setTimes(newTimes);
      console.log('Updated times state:', newTimes);
